...
{
    int tupleinterval, max_packets, sensor_tupleversion, tuplecounter, custom_flag, bad_packets_time, bad_packets, dbmsignal_limit, seqinterval, timenormalizer, totptimeinterval;
    ...
    seqinterval = *(args->seq_interval);
    timenormalizer = *(args->time_normalizer);
    totptimeinterval = *(args->totp_time_interval);
    max_packets = *(args->max_packets);
    sensor_tupleversion = *(args->sensor_tupleversion);
    tupleinterval = *(args->tupleinterval);
    ...
    generate_tuple(interface_name, queue, max_packets, sensor_tupleversion, custom_flag, tuplecounter, sensor_id, sensor_token, bad_packets_time, bad_packets, dbmsignal_limit, tupleinterval, seqinterval, timenormalizer, totptimeinterval);
}
