...
void got_pkg(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
	...
	int totptimeinterval = conf->totptimeinterval;
        ...
        sensor_token = conf->sensor_token;
	...
	if (rd.signal > dbmsignal_limit){
        	if (sensor_tupleversion == 1){
        	...
        	else if (sensor_tupleversion == 4){
        	        conf->tuplecounter++;
                    //totp = generateTOTPUsingTimestamp(sensor_id, 8, header->ts.tv_sec);
                    char float_str[25];
                    sprintf(float_str,"%ld.%.6ld", header->ts.tv_sec, header->ts.tv_usec);
        	        ...
        	        sprintf(tuple,"%d,%s,%d,%d,%ld.%.6ld,%02x:%02x:%02x:%02x:%02x:%02x,%d,%s,%d,%d\n",sensor_tupleversion, sensor_id, tuplecounter, queue->size, header->ts.tv_sec, header->ts.tv_usec, \
        	                                        rd.sa[0],rd.sa[1],rd.sa[2],rd.sa[3],rd.sa[4],rd.sa[5],rd.signal,ssid,sensor_customflag, totp);
                    //printf("%s\n",tuple);
        	}
        	else {
...
char * generate_tuple(char * device_name, struct Queue* queue, int max_packets, int sensor_tupleversion, int custom_flag, int tuplecounter, char *sensor_id, char *sensor_token, int bad_packets_time, int bad_packets, int dbmsignal_limit, int tupleinterval, int seqinterval, int timenormalizer, int totptimeinterval)
{
        ...
        char totp_key_str[1000];
        char totp_key_str_cache[1000];
        char tuple_str1[1000];
        char tuple_str2[1000];
        char tuple_str3[1000];
        char tuple_str4[1000];
        int tuple_count =0;
        /// PC
        int totp;
        long timestamp;
        ...
        conf.custom_flag = custom_flag;
        conf.sensor_id = sensor_id;
        conf.sensor_token = sensor_token;
	    ...
	    conf.seqinterval = seqinterval;
	    conf.timenormalizer = timenormalizer;
	    conf.totptimeinterval = totptimeinterval;
//#ifdef PC
//        int cache_hit = 0;
//        printf("#################\ntest-start-time %d\n", (int)time(NULL));
//        while(tuple_count < 20000000){
//        //printf("tupleinterval %d\n", tupleinterval);
//        usleep(1000 * tupleinterval);
//        //printf("tuple created\n");
//        int timestamp = (int)time(NULL);
//        int normalized_ts = (timestamp / 30) * 30;
//        //normalized_ts = normalized_ts + 30;
//        sprintf(totp_key_str,"%s%d", sensor_id, tuple_count / 1000 );
//        //sprintf(totp_key_str,"12345678901234567890", sensor_id, tuple_count / 1000 );
//        //normalized_ts = 2000000000;
//        if (strcmp(totp_key_str,totp_key_str_cache) != 0){
//            sprintf(totp_key_str_cache,"%s", totp_key_str);
//        }else{ cache_hit++;}
//        totp = generateTOTPUsingTimestamp(totp_key_str_cache, 8, normalized_ts);
//        printf("%d\n", totp);
//        //totp = generateTOTPUsingTimestamp(totp_key_str_cache, 8, 1111111111);
//        //printf("%d\n", totp);
//        //totp = generateTOTPUsingTimestamp(totp_key_str_cache, 8, 1111111109);
//        //printf("%d\n", totp);
//        //totp = generateTOTPUsingTimestamp(totp_key_str_cache, 8, 2000000000);
//        //printf("%d\n", totp);
//        //totp = generateTOTPUsingTimestamp(totp_key_str_cache, 8, 20000000000);
//        //printf("%d\n", totp);
//
//        /* open capture device */
...
//
//
//        //totp = generateTOTPUsingTimestamp(totp_key_str, 8, timestamp);
//        sprintf(tuple_str4, "4,%s,%d,%d,%d,4c:bb:58:33:2c:d1,-86.0,Inorbit,1,%d\n",sensor_id,tuple_count, queue->size, timestamp, totp);
//        sprintf(tuple_str3, "%s3,%s,%d,%d,%d,4c:bb:58:33:2c:d1,-86.0,Inorbit,1\n",tuple_str4,sensor_id,tuple_count, queue->size, timestamp);
//        sprintf(tuple_str2, "%s2,%s,%d,%d,%d,4c:bb:58:33:2c:d1,-86.0,Inorbit\n",tuple_str3,sensor_id,tuple_count, queue->size, timestamp);
//        sprintf(tuple_str1, "%s1,%s,%d,%d,%d,4c:bb:58:33:2c:d1,-86.0\n",tuple_str2,sensor_id,tuple_count, queue->size, timestamp);
//        printf("%s\tcache hit %d\n", tuple_str1, cache_hit);
//        enqueue(queue, tuple_str1, 0);
//        tuple_count++;
//        }
//        printf("test-end-time %d\t cachehit %d\t tuplecount %d\n", (int)time(NULL), cache_hit, tuple_count);
//        //tuple_count = 0;
//        //printf("test-start-time %d\n", (int)time(NULL));
//        //while(tuple_count < 20000000){
//        ////printf("tupleinterval %d\n", tupleinterval);
//        ////usleep(1000 * tupleinterval);
//        ////printf("tuple created\n");
//        //int timestamp = (int)time(NULL);
//        //int normalized_ts = timestamp - (timestamp % 30);
//        //normalized_ts = normalized_ts + 30;
//        //sprintf(totp_key_str,"%s%d%d", sensor_id, normalized_ts, tuple_count / 1000 );
//        ////if (strcmp(totp_key_str,totp_key_str_cache) != 0){
//        ////    sprintf(totp_key_str_cache,"%s", totp_key_str);
//        ////}else{ cache_hit++;}
//        ////totp = generateTOTPUsingTimestamp(totp_key_str_cache, 8, timestamp);
//        //totp = generateTOTPUsingTimestamp(totp_key_str, 8, timestamp);
//        //sprintf(tuple_str4, "4,%s,%d,%d,%d,4c:bb:58:33:2c:d1,-86.0,Inorbit,1,%d\n",sensor_id,tuple_count, queue->size, timestamp, totp);
//        //sprintf(tuple_str3, "%s3,%s,%d,%d,%d,4c:bb:58:33:2c:d1,-86.0,Inorbit,1\n",tuple_str4,sensor_id,tuple_count, queue->size, timestamp);
//        //sprintf(tuple_str2, "%s2,%s,%d,%d,%d,4c:bb:58:33:2c:d1,-86.0,Inorbit\n",tuple_str3,sensor_id,tuple_count, queue->size, timestamp);
...
//        //tuple_count++;
//        //}
//        //printf("test-end-time %d\t tuplecount %d\n", (int)time(NULL), tuple_count);
//
////#else
